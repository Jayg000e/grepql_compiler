...
let check (globals, functions) =
  ...
  let built_in_decls = 
    let add_bind map (name, param_types, return_type) = StringMap.add name {
      typ = return_type;
      fname = name; 
  ...
  let check_function func =
    ...
    let rec expr = function
      ...
      | Count(e) -> (Int, SCount(check_strings_expr e))
      | Save(e1, e2) -> (Void, SSave(check_strings_expr e1, check_string_expr e2))
      | Load(e) -> (Strings, SLoad(check_string_expr e))
    ...
    let rec check_stmt = function
      ...
      (* Update the function's return type to match the type of the return expression *)
    func.typ <- t; 
    SReturn (t, e')
